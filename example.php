<?php
/**
 * Contoh Script Bridge API Dramadash via StreamAPI.web.id
 * ----------------------------------------------------
...
// Fungsi bantuan buat manggil endpoint Dramadash di sistem Bridge
function callDramadashAPI($urlTujuan, $token) {
...
// UJI COBA KODE
// -----------------------------------------------------------------
$baseUrl = "https://streamapi.web.id/p/dramadash/api/v1";

// Contoh 1: Fitur Cari Film Dramadash
echo "\n=====================[ 1. Pencarian Dramadash ]==================\n";
$kataKunci = urlencode("cinta");
$halaman = 1;
$urlCari = "{$baseUrl}/search/{$kataKunci}/{$halaman}?lang=in";

$hasilCari = callDramadashAPI($urlCari, $API_TOKEN);
if ($hasilCari) {
...
$hasilDetail = callDramadashAPI($urlDetail, $API_TOKEN);
if ($hasilDetail) {
...
$hasilNonton = callDramadashAPI($urlNonton, $API_TOKEN);
if ($hasilNonton) {
...
echo "▶▶▶ https://streamapi.web.id/docs/dramadash ◀◀◀\n\n";
